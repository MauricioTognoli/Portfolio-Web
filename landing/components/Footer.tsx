  ...
  return (
    <footer className="flex flex-wrap items-center justify-between gap-4 border-t border-mt-border bg-mt-bg px-12 py-7 transition-colors duration-300 max-[900px]:px-6 max-[560px]:flex-col max-[560px]:items-start max-[560px]:px-5 max-[560px]:py-6">
      ...
      <ul className="flex flex-wrap list-none gap-[22px]">
        <li><a className="text-xs text-mt-text-30 no-underline transition-colors duration-200 hover:text-mt-text-60" href={cvLink} target="_blank" rel="noopener noreferrer">{t.footer.cv}</a></li>
        ...
        <li><a className="text-xs text-mt-text-30 no-underline transition-colors duration-200 hover:text-mt-text-60" href="/#components">{t.footer.components}</a></li>
      </ul>
    </footer>
  );
};
